...
export async function classifyWithLlm(message: string): Promise<AgentName | null> {
  ...
  const systemPrompt = [
    ...
    "finance     — trading intelligence, stocks, ETFs, options, futures, crypto, forex, TradingView, paper trades, trade theses, backtesting, journals, scanners, Bank & Sweep, Trades By Sci, TopStep, position management, wealth planning, yield, portfolio strategy.",
    "",
    "Important: Any request containing a URL, website, browse, visit, inspect, current, latest, or news intent must route to research unless the user is clearly asking to use stored trading knowledge or review a trading record.",
    "",
