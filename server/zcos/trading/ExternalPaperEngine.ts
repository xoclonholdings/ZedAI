...
const PAPER_PROVIDERS = ["tradovate", "tradingview", "lucid"];
...
export async function getExternalPaperReport(userId: string): Promise<ExternalPaperReport> {
  ...
  const summary = !providerConnected
    ? "Connect a paper/demo provider (Tradovate demo, TradingView paper, or Lucid) so Zed can prove the strategy on real platform rails."
    : passed
