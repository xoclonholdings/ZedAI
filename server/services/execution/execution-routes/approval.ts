...
export function registerApprovalEndpoints(app: Express): void {
  ...
  app.post("/api/approval/decide", isAuthenticated, async (req: any, res: Response) => {
    try {
      const { task_id, action, reason, manual_handling_notes } =
        req.body || {};
      if (!task_id || !action) {
      ...
      const result = await ApprovalDecisionHandler.decide({
        task_id,
        decided_by: user_id,
        decider_role: req.user?.claims?.isAdmin ? "admin" : "user",
        action,
        reason,
  ...
  app.get("/api/approval/notifications", isAuthenticated, async (req: any, res: Response) => {
    try {
      const user_id = userIdFrom(req);
      const role = req.user?.claims?.isAdmin && req.query.role === "admin" ? "admin" : "user";
      const items = await ApprovalNotificationService.list({
        ...
        recipient_id: role === "user" ? user_id : undefined,
  ...
  app.post(
    ...
    async (req: any, res: Response) => {
      try {
        const ok = await ApprovalNotificationService.markReadForOwner(
          req.params.id,
          ownerContextFrom(req),
          Boolean(req.user?.claims?.isAdmin),
        );
        res.json({ ok });
      } catch (err: any) {
