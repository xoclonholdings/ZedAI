...
async function buildSystemMessage(mode: "chat" | "agent", userContext?: UserContext): Promise<string> {
  let systemContent = "You are ZED, an advanced AI assistant with document processing capabilities.";
  
  try {
    // Load enhanced personality if user has admin access
    if (userContext?.accessLevel === 'admin' && userContext?.personalityMode === 'enhanced') {
      console.log('[SYSTEM MESSAGE] Loading enhanced admin personality');
      const enhancedPersonality = await PersonalityLoader.loadPersonalityForUser(userContext);
      
      if (enhancedPersonality) {
        return PersonalityLoader.buildEnhancedSystemMessage(enhancedPersonality, mode);
      }
    }
    
    // Fallback to standard memory system
...
export async function generateChatResponse(
  ...
  try {
    // Load enhanced system message with personality
    let systemContent = await buildSystemMessage(mode, userContext);
    ...
    console.log(`[AI ROUTER] Using ${aiProvider.provider} for ${mode} mode${userContext?.accessLevel === 'admin' ? ' (Enhanced Admin Mode)' : ''}`);
    ...
    if (aiProvider.provider === "ollama") {
      try {
        ...
        return await generateLocalResponse(messages, mode, userContext);
      }
    }
    ...
    const response = await generateLocalResponse(messages, mode, userContext);
...
async function generateLocalResponse(messages: ChatMessage[], mode: "chat" | "agent", userContext?: UserContext): Promise<string> {
  const lastUserMessage = messages.filter(m => m.role === "user").pop()?.content || "";
  const conversationHistory = messages.slice(-10); // Keep last 10 messages for context
  
  // Load enhanced personality context for admin users
  let enhancedContext = null;
  if (userContext?.accessLevel === 'admin' && userContext?.personalityMode === 'enhanced') {
    try {
      enhancedContext = await PersonalityLoader.loadPersonalityForUser(userContext);
      console.log('[LOCAL AI] Using enhanced admin personality for local response');
    } catch (error) {
      console.error('[LOCAL AI] Error loading enhanced context:', error);
    }
  }
  ...
  if (userInput.includes("code") || userInput.includes("programming") || userInput.includes("api")) {
    return generateTechnicalResponse(lastUserMessage, mode, enhancedContext);
  }
  
  // File processing queries
  if (userInput.includes("file") || userInput.includes("upload") || userInput.includes("document")) {
    return generateFileResponse(lastUserMessage, mode, enhancedContext);
  }
  
  // Database/storage queries
  if (userInput.includes("database") || userInput.includes("storage") || userInput.includes("data")) {
    return generateDatabaseResponse(lastUserMessage, mode, enhancedContext);
  }
  
  // System status queries
  if (userInput.includes("status") || userInput.includes("working") || userInput.includes("test")) {
    return generateStatusResponse(lastUserMessage, mode, enhancedContext);
  }
  
  // Default intelligent response
  return generateContextualResponse(lastUserMessage, conversationHistory, mode, enhancedContext);
}

function generateTechnicalResponse(userMessage: string, mode: string, enhancedContext?: any): string {
  let baseResponse = "";
  
  if (mode === "agent") {
    baseResponse = `I'm analyzing your technical query: "${userMessage}"
...
**Next Steps:** Specify which technical aspect you'd like me to analyze further.`;
    ...
    baseResponse = `I understand you're asking about: "${userMessage}"
