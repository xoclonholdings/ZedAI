import { readAppliedGraph } from "./object-memory/store";

/**
 * Workspace-memory retrieval.
 *
 * Whenever Zed works on a request inside a workspace, it must first run
 * through that workspace's own knowledge (the slice of core memory tagged
 * with the workspace) so its work is grounded in what you taught it there.
 *
 * Knowledge lives in the single core object-memory graph; objects added
 * through a workspace library carry `properties.workspace = <slug>`. This
 * service pulls that slice, ranks it against the request, and formats a
 * prompt block that gets injected ahead of general knowledge.
 */
...
export async function buildWorkspaceMemoryContext(
  ...
  const graph = await readAppliedGraph().catch(() => null);
