"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.default = ChatMessage;
var card_1 = require("@/components/ui/card");
var badge_1 = require("@/components/ui/badge");
var lucide_react_1 = require("lucide-react");
var zLogoPath = "data:image/svg+xml;base64,";
function ChatMessage(_a) {
    var message = _a.message;
    var isUser = message.role === "user";
    var getFileIcon = function (mimeType) {
        if (mimeType.startsWith('image/'))
            return <lucide_react_1.Image size={14}/>;
        if (mimeType.includes('csv') || mimeType.includes('excel'))
            return <lucide_react_1.BarChart size={14}/>;
        return <lucide_react_1.FileText size={14}/>;
    };
    if (isUser) {
        return (<div className="flex items-start space-x-4 flex-row-reverse max-w-4xl mx-auto">
        <div className="w-10 h-10 bg-gradient-to-br from-slate-600 to-slate-700 rounded-2xl flex items-center justify-center flex-shrink-0">
          <lucide_react_1.User className="text-white" size={18}/>
        </div>
        <div className="flex-1 max-w-2xl">
          <card_1.Card className="zed-glass border-white/20 p-6">
            <div className="prose prose-sm max-w-none">
              <p className="whitespace-pre-wrap text-foreground leading-relaxed">{message.content}</p>
            </div>
            
            {message.attachments && message.attachments.length > 0 && (<>
                {message.attachments.map(function (file, index) { return (<div key={index} className="flex items-center space-x-3 p-2 rounded-xl bg-white/5">
                    <div className="text-cyan-400">
                      {getFileIcon(file.mimeType)}
                    </div>
                    <span className="truncate text-sm text-foreground">{file.name}</span>
                    <badge_1.Badge variant="outline" className="text-xs border-cyan-500/30 text-cyan-300">
                      {(file.size / 1024 / 1024).toFixed(1)} MB
                    </badge_1.Badge>
                  </div>); })}
              </>)}
          </card_1.Card>
        </div>
      </div>);
    }
    return (<div className="flex items-start space-x-4 max-w-4xl mx-auto">
      <div className="flex-1">
        <card_1.Card className="zed-message p-6 ml-4">
          <div className="flex items-center space-x-2 mb-4">
            <img src={zLogoPath} alt="Z" className="w-4 h-4"/>
            <span className="text-lg font-semibold bg-gradient-to-r from-pink-500 via-purple-500 to-blue-500 bg-clip-text text-transparent">
              ZED
            </span>
            <badge_1.Badge variant="outline" className="text-xs border-purple-500/30 text-purple-300">
              <lucide_react_1.Sparkles size={10} className="mr-1"/>
              AI Assistant
            </badge_1.Badge>
          </div>
          
          <div className="prose prose-sm max-w-none">
            <p className="whitespace-pre-wrap leading-relaxed">{message.content}</p>
          </div>
        </card_1.Card>
      </div>
    </div>);
}
