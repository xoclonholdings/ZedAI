...
export default function MemoryPanel({ isOpen, onClose, username }: MemoryPanelProps) {
    ...
    const loadMemoryData = async () => {
        ...
        try {
            ...
            const response = await apiRequest(`/api/zed/memory/${targetUser}`, "GET");
    ...
    const filteredEntries = getAllEntries().filter(entry => {
        ...
        const matchesSearch = searchQuery === '' ||
            ('title' in entry && entry.title && entry.title.toLowerCase().includes(searchQuery.toLowerCase())) ||
            (entry.type === 'upload' && 'filename' in entry && entry.filename.toLowerCase().includes(searchQuery.toLowerCase()));
