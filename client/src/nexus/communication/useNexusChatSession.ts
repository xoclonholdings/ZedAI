...
import { useNexusConversationController, type NexusConversationController } from "./useNexusConversationController";
import {
 ...
 * client-action resolution that lets ZAR's own responses drive real Nexus
 ...
 */
export function useNexusChatSession(conversationId?: string | null, options?: { onModeAction?: (modeId: string) => void }) {
  const [, navigate] = useLocation();
  const { capabilityRegistry, communicationLayer, navigateToNode, snapshot } = useNexus();
  ...
  const handleBeforeSend = useCallback((message: string) => {
    const exactAction = resolveDeterministicNexusClientAction(message, snapshot, capabilityRegistry, communicationLayer);
  ...
  const controller: NexusConversationController = useNexusConversationController({
