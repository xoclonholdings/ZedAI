...
import { NexusApplicationSurface } from "../components/NexusApplicationSurface";
import { NexusConstellation } from "../components/NexusConstellation";
import { NexusConversationSurface } from "../components/NexusConversationSurface";
...
interface NexusRootPageProps {
  ...
  /** True on /chat routes — the communication room was opened intentionally. */
  readonly communicationOpen?: boolean;
}
...
export default function NexusRootPage({
  ...
  // /nexus is the portal: constellation + one adaptive composer, almost
  // empty. /nexus/:nodeId is an intentionally opened application: the
  // environment shifts into that node's living surface with real state.
  const insideApplication = Boolean(routeNodeId);
        ...
        <div className="mx-auto flex max-w-6xl items-center justify-between gap-4 py-4">
          ...
          {insideApplication && (
            <button
              ...
              className="rounded-full border border-white/10 bg-white/[0.035] px-3 py-1.5 text-[12px] text-white/58 backdrop-blur-md transition hover:border-cyan-200/30 hover:text-white"
            >
              Back to Nexus
            </button>
          )}
      ...
      <main className="mx-auto flex max-w-6xl flex-col gap-4 px-4 pb-safe sm:px-6">
        {communicationOpen ? (
          <NexusConversationSurface
            conversationId={communicationConversationId}
            variant="full"
          />
        ) : (
          <>
