...
import { extractNexusClientActions } from "../actions/NexusClientActions";
import type { Message } from "@shared/schema";

test("Nexus communication dispatch preserves prompts and exposes structured client actions", async () => {
  ...
  globalThis.fetch = (async (_input: RequestInfo | URL, init?: RequestInit) => {
    ...
    return new Response(JSON.stringify({
      ...
      metadata: {
        nexusClientActions: [{ type: "focus-node", nodeId: "memory" }],
      },
    }), {
      status: 200,
  ...
  assert.deepEqual(extractNexusClientActions(responses[0]), [
