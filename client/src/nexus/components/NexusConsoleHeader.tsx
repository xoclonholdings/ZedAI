...
import { NexusLiveClock, NexusLiveDate, NexusLiveQuote } from "./NexusLiveGreeting";

export interface NexusHeaderContext {
 ...
 */
export function NexusConsoleHeaderBrand({
  onWordmarkClick,
  wordmarkAriaLabel,
  context,
}: {
  ...
  readonly context: NexusHeaderContext | null;
}) {
  return (
...
export function NexusConsoleHeaderTelemetry({ visible }: { readonly visible: boolean }) {
  return (
    <div className="flex flex-col items-end">
      <div className="flex h-9 items-center">
        <NexusLiveClock visible={visible} />
      </div>
      <div className="flex h-4 items-center">
        <NexusLiveDate visible={visible} />
      </div>
      <div className="flex h-6 items-center">
        <NexusLiveQuote visible={visible} />
      </div>
    </div>
  );
}
