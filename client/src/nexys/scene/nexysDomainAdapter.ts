...
import { resolveNexysIconComponent } from "../components/NexysIcon";
import type { NexysNodeDefinition } from "../graph/types";

/**
 * Translates real root-node manifest data into the shape NexysCore's scene
 ...
 */
export function nexysDomainsFromRootNodes(nodes: readonly NexysNodeDefinition[]): NexysDomain[] {
  return nodes.map((node) => {
    ...
    return {
      ...
      icon: resolveNexysIconComponent(node.metadata.visual.icon),
    };
  });
}
