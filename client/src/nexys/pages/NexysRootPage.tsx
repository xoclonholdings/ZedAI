...
import NexysCore from "../components/NexysCore";
import { NexysConsoleHeaderBrand, NexysConsoleHeaderTelemetry } from "../components/NexysConsoleHeader";
import { NexysDeveloperInspector } from "../components/NexysDeveloperInspector";
import { NexysHubOverlay } from "../components/NexysHubOverlay";
import type { NexysDomain } from "../components/NexysCore";
import { isNexysRootNodeId, routeForNexysNode } from "../graph/rootConstellation";
import { nexysDomainsFromRootNodes } from "../scene/nexysDomainAdapter";
import { canUseNexysWebgl, type NexysInteractionStage } from "../scene/nexysSceneContract";
import { useNexys } from "../state/NexysProvider";
import { createFocusedNodeView, shouldShowNexysDeveloperInspector } from "../viewport/NexysViewportModel";
import { resolveNexysEnterAction, resolveNexysStageOnRouteChange } from "./nexysInteractionStageModel";

/** How long the "target" beat holds before Orbit's camera movement begins - brief but real, not instant. */
const TARGET_BEAT_MS = 16;
/**
 * NexysCore's own camera dolly doesn't expose a settle callback (it's the
...
export default function NexysRootPage() {
  const params = useParams<{ nodeId?: string }>();
  const [location, navigate] = useLocation();
  const { focusNode, clearFocus, snapshot, viewportSnapshot, capabilityRegistry } = useNexys();
  const routeNodeId = isNexysRootNodeId(params.nodeId) ? params.nodeId : null;
  const hasUnknownRouteNode = Boolean(params.nodeId && !routeNodeId);
  const showInspector = useMemo(() => shouldShowNexysDeveloperInspector({
  ...
  const [ambientDomain, setAmbientDomain] = useState<NexysDomain | null>(null);
  ...
  const [stage, setStage] = useState<NexysInteractionStage>(
    () => resolveNexysStageOnRouteChange({ routeNodeId, reducedMotion: false, isFirstRenderForMount: true }).stage,
  );
  ...
  useEffect(() => {
    if (hasUnknownRouteNode) navigate("/nexys");
  ...
  useEffect(() => {
    ...
    const decision = resolveNexysStageOnRouteChange({
      routeNodeId,
      reducedMotion,
  ...
  // reach here at all.
  const handleDomainSelect = useCallback((domain: NexysDomain) => {
    navigate(routeForNexysNode(domain.id));
  }, [navigate]);

  const goHome = useCallback(() => navigate("/nexys"), [navigate]);

  // STATE 4 (Enter): only trigger Warp when the action actually leaves Nexys for
  // a real workspace. Some capability actions still route back into /nexys/...
  ...
  const enterWorkspace = useCallback((route: string | null) => {
    const decision = resolveNexysEnterAction(route, warpingRef.current);
  ...
  const handleFocusedTap = useCallback((domain: NexysDomain) => {
    ...
    const selfLoop = `/nexys/${domain.id}`;
  ...
  return (
    <ConsoleShell
      ...
      headerRightExtra={<NexysConsoleHeaderTelemetry visible={stage === "home"} />}
    >
      {/* Celestial system - fills the entire viewport. This IS the application screen. */}
      <div className="absolute inset-0" data-nexys-region="scene">
        {webgl ? (
          <NexysCore
            domains={domains}
          ...
          <div className="flex h-full w-full items-center justify-center px-6 text-center text-sm text-white/50">
            Nexys needs a WebGL-capable browser to render the universe.
          </div>
        )}
      </div>
      ...
      {/* STATE 3 (Hub): the gateway reveal, still inside Nexys. */}
      {showHub && <NexysHubOverlay onBack={goHome} />}

      {showInspector && <NexysDeveloperInspector />}
    </ConsoleShell>
  );
}
