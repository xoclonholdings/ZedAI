...
app.post('/ai', async (req, res) => {
    ...
    try {
        ...
        if (/math|sum|calculate|logic|reason|summary|summarize|multi-step|context|combine/i.test(input)) {
            ...
            const reasoned = (0, reasoning_1.reasonOverHistory)(input);
...
app.post('/reason', (req, res) => {
    try {
        ...
        const response = (0, reasoning_1.reasonOverHistory)(prompt);
        (0, memory_1.saveTurn)(prompt, response);
        (0, adaptiveLearning_1.updateState)(prompt, response);
        res.json({ response });
    }
    catch (err) {
        const errorMsg = err instanceof Error ? err.message : String(err);
